...
def performance_test_data(django_db_setup, django_db_blocker):
    with django_db_blocker.unblock():
        ...
        order_items = [
            baker.prepare(OrderItem, order=order, product=product, quantity=1)
            for order in orders
        ]
...
class TestBulkUpdatePerformance:
    ...
    def test_bulk_update_order_total_performance(self, performance_test_data):
        ...
        expected = sum(
            item.quantity * item.product.price for item in sample.orderitem.all()
        )
...
class TestBulkUpdateOrderTotals:
    def test_bulk_update_order_total_small(self, order_test_data):
        order_totals = (
            OrderItem.objects.filter(order=dm.OuterRef("pk"))
            .annotate(
            ...
            .values("order")
            .annotate(total=dm.Sum("line_total"))
            .values("total")
        )
        ...
        for order in Order.objects.all():
            expected = sum(
                item.quantity * item.product.price for item in order.orderitem.all()
            )
